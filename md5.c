...
static inline
uint32_t N2HL(const uint8_t * b)
{
    register uint32_t q = *(uint32_t*)b;
    //__builtin_memcpy(&q,b,4);
...
static inline uint8_t HL2N(uint32_t * v, int i)
{
...
#define BLK_SIZE 64
void md5_updt(MD5Ctx * ctx, const uint8_t * msg, unsigned int mlen)
{
...
void md5_fini(MD5Ctx * ctx, uint8_t * tag, unsigned int tlen)
{
