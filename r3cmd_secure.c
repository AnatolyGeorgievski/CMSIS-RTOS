...
/*! \brief Функция (0x0С) — отсылка пакета с аутентификацией
	...
	Данные передаются в открытом виде, с выработкой имитовставки и последующим усечением результата до 32 бит.
	Запрос: [ 0x0С || InvokeId8 || SecuredData || MAC32]
	Отклик: [ 0x0С || InvokeId8 || SecuredData || MAC32]
...
int r3_get_secured(uint8_t* buffer, int* length)
{
	...
	int len=0;
	uint8_t invoke_id = buffer[len++];
    unsigned int code = buffer[len++];
    while (r3cmd < r3cmd_stop)
    {
        if (code == r3cmd->code){
			...
			*length+=1;
			uint32_t* Key = Keys[key_id].private_key;
			uint64_t cmac= magma_cmac(Key, buffer, *length);
			*(uint64_t*)(buffer+*length) = cmac; *length+=CMAC_LEN;
            return *length;
        }
        r3cmd++;
    }
...
int r3_set_secured(uint8_t* buffer, int* length)
{
	int len = 0;
	uint8_t invoke_id = buffer[len++];
	uint8_t code = buffer[len++];
	uint8_t key_id = 0;
	uint32_t* Key = Keys[key_id].private_key;
	int data_len = *length-CMAC_LEN;
	uint64_t cmac= magma_cmac(Key, buffer, data_len);
	if (cmac==*(uint64_t*)&buffer[data_len]){
		
		// найти по коду
		buffer[1] = R3_cmd_snmp_recv(code, buffer+2, *length);
		//memcpy(buf+len, cmac, 8); len+=8;
	}
	return R3_ERR_ABORT;
}

R3CMD(R3_SNMP_SET_KEYINFO, r3_set_keyinfo);
